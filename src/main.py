...
from src.posture_image_logger import initialize_folders, save_posture_image
...
datasets = api.dataset_list(search="dogs", page=1)
for d in datasets:
...
import os
assets_path = os.path.join(os.path.dirname(__file__), "..", "assets", "bad_posture_alert.wav")
beep = pygame.mixer.Sound(os.path.abspath(assets_path))
...
TREND_LOG_PATH = os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "data", "posture_trend_log.csv"
))

file_exists = os.path.exists(TREND_LOG_PATH)
log_file = open(TREND_LOG_PATH, mode='a', newline='')
...
# Locked path to data/posture_dataset.csv
POSTURE_CSV_PATH = os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "data", "posture_dataset.csv"
))

#data logger
def log_posture_sample(features, label, frame=None,filename = POSTURE_CSV_PATH):
