...
class App(QMainWindow):
    def __init__(self):
       ...
       # main_layout.addWidget(tab_container)  
        ...
        # Try to load pixmap only if file exists and it's a valid image.
        if os.path.exists(background_path):
            candidate = QPixmap(background_path)
            if not candidate.isNull():
                self.bg_pixmap = candidate
                palette = self.palette()
                palette.setBrush(
                    QPalette.Window,
                    QBrush(self.bg_pixmap.scaled(self.size(),
                ...
                print(f"⚠️ Background image found but failed to load: {background_path}")
                # fallback to default system background (do not force a brush)
                self.setAutoFillBackground(False)
                # Optionally set a neutral color if you want a consistent look:
                # self.setStyleSheet('background-color: #F0F0F0;')
        else:
            print(f"⚠️ Background image not found: {background_path}. Using default background.")
            self.setAutoFillBackground(False)
            # Optionally set a neutral color:
            # self.setStyleSheet('background-color: #F0F0F0;')
    ...
    def resizeEvent(self, event):
        if hasattr(self, "bg_pixmap") and self.bg_pixmap is not None and not self.bg_pixmap.isNull():
            palette = self.palette()
