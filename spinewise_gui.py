# SpineWise Capstone 2 Project - Credits to Emdya Permuy for the concept and for her leadership during Capstone 1.
# Past Members:
# Emdya Permuy-Llovio (Capstone 1 Team Leader, Backend, Frontend)
# Jake Rodriguez (Backend, Notification System Foundation)
# Members Since Cap. 1:
# Javier Brasil (Backend, Dataset Training, Testing, Machine Learning, Live Coach)
# Juan Mieses (Capstone 2 Team Leader, Frontend, UI/UX, Analytics Tab)
# John Pena (Backend, Dataset Creation, Machine Learning)
# New Members:
# Oleh Krainyk (Frontend, UI/UX, Recommendations Tab, Code Optimizer)
# Jason Pena (Frontend, UI/UX, Analytics Tab)
# SpineWise description goes here

# Core essential imports.
import os, queue, cv2, csv, json, datetime, re, time, numpy as np, pandas as pd, mediapipe as mp, threading, backend, speech_recognition as sr
from backend import ( analyze_posture, get_pose_landmarker, get_face_landmarker, draw_landmarks, normalize_lighting, set_gui_mode, get_recommendation_context )

# Theme consistency imports.
...
# Voice config import for voice commands.
from voice_config import voice_config

# PyQt5 imports.
from PyQt5.QtNetwork import ( QNetworkRequest, QNetworkAccessManager )
from PyQt5.QtWidgets import ( QLabel, QPushButton, QStackedWidget, QButtonGroup, QRadioButton, QSizePolicy, QFrame, QVBoxLayout, QWidget, QTabWidget, QMainWindow, QFileDialog, QTextEdit, QDoubleSpinBox, QScrollArea, QSpinBox, QHBoxLayout, QCheckBox, QFormLayout, QSlider, QGroupBox, QProgressBar, QTableWidgetItem, QTableWidget, QGridLayout, QHeaderView, QAction, QLineEdit )
from PyQt5.QtGui import ( QImage, QDesktopServices, QPixmap, QFont, QIcon, QColor )
from PyQt5.QtCore import ( Qt, QUrl, QSize, QThread, pyqtSignal, QTimer, QObject, QTime, QDate, QByteArray )
...
# GraphThread handles writing the posture_trend_log.csv data and the stats.json data into the analytics tab.
class GraphThread(QThread):
    ...
    def __init__(self):
        ...
        self.tab_active = self.default = False
        ...
        self.good_color = "#10B981"        # good posture
        self.moderate_color = "#2563EB"    # moderate
        self.bad_color = "#EF4444"         # bad
        ...
        self.canvas.ax1 = self.figure.add_subplot(111)
        self.canvas.ax1.autoscale(enable=False)

        self.xdata = [0]
        self.ydata = [0]
        self.colors = []

        # Apply initial styling to the axis
        ...
        try:
            with open("stats.json", "r") as json_file:
                ...
                self.avg_counter = data['counter']
                self.totalTime = QTime.fromString(data['time'], "hh:mm:ss")
            ...
            if self.tab_active:
                ...
                try:
                    ...
                    if (self.intervalMax == 0):
                        self.intervalMax = round(self.xdata[(len(self.ydata) - 1)] / 3)
                        if self.intervalMax < 6:
                            self.intervalMax = 6
                ...
                for row in csv_reader:
                    if len(row) >= 6 and row[-1].replace('.', '').replace('-', '').isdigit():
                        try:
                            ...
                            self.ydata.append(confidence)
                            self.xdata.append(self.xdata[-1] + interval)

                            if confidence >= 5:
                                self.colors.append(self.bad_color)
                            elif confidence <= 1:
                                self.colors.append(self.good_color)
                            else:
    ...
    def plot_on_main_thread(self):
        try:
            self.canvas.ax1.cla()
            self.canvas.ax1.invert_yaxis()

            if len(self.xdata) > 1:
                for i in range(1, len(self.xdata)):
                    x_seg = [self.xdata[i - 1], self.xdata[i]]
                    y_seg = [self.ydata[i - 1], self.ydata[i]]

                    if i - 1 < len(self.colors):
                        seg_color = self.colors[i - 1]
                    else:
                        seg_color = self.moderate_color  # sensible fallback

                    self.canvas.ax1.plot(x_seg, y_seg, linewidth=1.5, color=seg_color)

            scatter = self.canvas.ax1.scatter(
                self.xdata[1:], self.ydata[1:], c=self.colors, s=20
            )

            # This is the Legend that defines colors.
            good_circle = mpatches.Patch(
                color=self.good_color,
                label="Good\nPosture",
            )
            moderate_circle = mpatches.Patch(
                color=self.moderate_color,
                label="Moderate\nPosture",
            )
            bad_circle = mpatches.Patch(
                color=self.bad_color,
                label="Bad\nPosture",
            )

            self.canvas.ax1.legend(
                handles=[bad_circle, moderate_circle, good_circle],
                loc="center left",
                bbox_to_anchor=(1, 0.5),
            )

            # Hover tooltip.
            annotation = self.canvas.ax1.annotate(
                text="",
                xy=(0, 0),
                xytext=(15, 15),
            ...
            def motion_hover(event):
                ...
                if event.inaxes == self.canvas.ax1:
                    is_hovered, annotation_index = scatter.contains(event)
                    if is_hovered and len(annotation_index["ind"]) > 0:
                        location = scatter.get_offsets()[annotation_index["ind"][0]]
                        annotation.xy = location
                        text_label = f"({location[0]}, {location[1]})"
            ...
            self.canvas.ax1.set_ylabel("Posture Score")
            self.canvas.ax1.set_xlabel("Time (s)")
            self.canvas.ax1.grid(False)
            self.canvas.ax1.set_xlim(self.xmin, self.xmax)
...
class App(QMainWindow):
    def __init__(self):
        ...
        icon_path = "assets/icons/spinewiselogo.png"
        self.setWindowIcon(QIcon(icon_path))
        try:
            import ctypes
            myappid = 'spinewise.posture.app.1.0'  # arbitrary string
    ...
    def init_analytics_tab(self):
        ...
        self.today_avg_time = QLabel("0m 0s")
        self.today_avg_time.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: 28px; font-weight: bold;")
        time_layout.addWidget(self.today_avg_time)
        ...
        self.graph_thread.update_plot.connect(lambda: slider.setMaximum(self.graph_thread.xdata[-1] - 3))
        self.graph_thread.update_plot.connect(lambda: intervalSpinBox.setMaximum(self.graph_thread.intervalMax))
        ...
        """)
        reset_view.clicked.connect(lambda: slider.setValue(0))
        reset_view.clicked.connect(lambda: intervalSpinBox.setValue(5))
        reset_view.clicked.connect(self._refresh_analytics_data)
        graphs_layout.addWidget(reset_view)
        ...
        # Updating Avg Time
        if self.graph_thread.avg_counter > 1:
            seconds_val = QTime(0, 0, 0).secsTo(self.graph_thread.totalTime)
            seconds_val /= self.graph_thread.avg_counter
            time_str = str(datetime.timedelta(seconds=round(seconds_val)))

            parts = time_str.split(':')
            hours = int(parts[0])
            minutes = int(parts[1])
            seconds_val = int(parts[2])

            # Format nicely
            if hours > 0:
                self.today_avg_time.setText(f"{hours}h {minutes}m {seconds_val}s")
            else:
                self.today_avg_time.setText(f"{minutes}m {seconds_val}s")
    ...
    def _refresh_analytics_data(self):
        ...
        try:
            ...
            if os.path.exists("posture_trend_log.csv"):
                self.graph_thread.default = False
                self.graph_thread.intervalMax = 0
    ...
    def closeEvent(self, event):
        ...
        with open("stats.json", "r") as json_file:
            ...
            if (data['day'] == currentDay):
                data['counter'] += 1
                timestamp = self.graph_thread.totalTime.toString("hh:mm:ss")
            else:
                data['counter'] = 0
            data['time'] = timestamp
            data['day'] = currentDay
