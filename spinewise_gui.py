...
class GraphThread(QThread):  # Tried changing from QObject to QThread as a dummy fix
    ...
    def __init__(self):
        ...
        self.canvas.ax1 = self.figure.add_subplot()
        ...
        try:
            ...
            with open("stats.json", "w") as json_file:
                json.dump({"day" : datetime.date.today().day, "counter" : 0, "time": "00:00:00","frequency": {}}, json_file)
            try:
                ...
                for row in csv_reader:
                    if len(row) >= 6 and row[-1].replace('.', '').replace('-', '').isdigit():
                        try:
                            ...
                            if (confidence >= 5):
                                self.colors.append("firebrick")
                            elif (confidence <= 1):
                                self.colors.append("yellowgreen")
                            else:
    ...
    def plot_on_main_thread(self):
        try:
            self.canvas.ax1.cla()
            self.canvas.ax1.plot(self.xdata, self.ydata, linewidth = 1)
            scatter = self.canvas.ax1.scatter(self.xdata[1:], self.ydata[1:], c = self.colors, s = 20)

            good_circle = mpatches.Patch(color = "yellowgreen", label = "Good\nPosture")
            moderate_circle = mpatches.Patch(color = "royalblue", label = "Moderate\nPosture")
            bad_circle = mpatches.Patch(color = "firebrick", label = "Bad\nPosture")

            self.canvas.ax1.legend(handles = [bad_circle, moderate_circle, good_circle], loc='center left', bbox_to_anchor=(1, 0.5))

            # Creating annotation object which will be connected to the hover event below
            ...
            # Implement hover event to display annotations
            ...
            # hook hover event to function responsible for displaying x,y
            self.canvas.mpl_connect('motion_notify_event', motion_hover)
            
            self.canvas.ax1.set_title("Posture Score over Time")
