...
def analyze_posture(image, pose_landmarks, face_landmarks=None):
    ...
    clavicle_length = np.linalg.norm(
        np.array((left_shoulder.x, left_shoulder.y)) - np.array((right_shoulder.x, right_shoulder.y))).astype(float)
    ...
    poly = Polygon([(left_shoulder.x, left_shoulder.y), (right_shoulder.x, right_shoulder.y), (right_hip.x, right_hip.y), (left_hip.x, left_hip.y)])
    torso_area = shapely.area(poly)
    ...
    if is_calibrating:
        ...
        if elapsed < countdown_duration:
            ...
            calibration_data["torso_areas"].append(torso_area)
    ...
    if calibrated_thresholds and not is_calibrating:
        ...
        torso_area_avg = avg("torso_areas")
        torso_area_percentage = (torso_area - torso_area_avg) / torso_area_avg

        if abs(torso_area_percentage) > 0.15:
            identifier = "far" if torso_area_percentage < 0 else "close"
            cv.putText(image, f"Too {identifier}, please move back or recalibrate!", (30, 120), cv.FONT_HERSHEY_SIMPLEX, 0.6,
                       (25, 25, 200), 2)
