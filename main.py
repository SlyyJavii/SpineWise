...
# Lighting normalization function to reduce effect of poor lighting

def normalize_lighting(frame):
    # Convert the image to LAB color space to work with lightness more accurately
    ...
    normalized = cv.cvtColor(merged, cv.COLOR_LAB2BGR)
    return normalized
...
with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
    ...
    while cap.isOpened():
        ...
        if success:
            frame = normalize_lighting(frame)  # Normalize lighting for consistent posture detection
        if not success:
