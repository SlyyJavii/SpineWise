...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            # 5/20/25 Juan Fix: Turned z_diffs into absolute value functions. Why? Negative values were throwing
            # the side hunch and side lean throw-ups into a funk--they'd never throw their appropriate values,
            # leaving posture either always "Good" or, in some cases, always "Hunched". You'd never see "lean".
            # This might be a poor fix but it works, I'm just skeptical of it.
            z_diff_head_nose = abs(nose.z - left_hip.z)
            z_diff_head_eye = abs(mid_eye_z - left_hip.z)
            ...
            # 5/20/25 Juan Fix: Flipped the signs. Partly due to personal understanding of how the numbers work or
            # should work(?). If abs_val'd z-diffs go over thresholds, warnings/bad is thrown.
            # Again, this might be a huge flimsy bandaid fix, or it might be really good.
            # I'm just worried I've been thinking of only this block, so it could affect rest of code adversely.
            # Note: abs'ing the z-diffs here actually destroys this block. Why? I have no clue. I think NOT abs'ing them messes with something behind the scenes,
            # causing this entire block to be dysfunctional. As a result, the z-diffs themselves were directly abs()'d in their original block.
            elif calibrated_thresholds and mode == "side":
                if z_diff_head_nose > calibrated_thresholds["nose_bad"] or z_diff_head_eye > calibrated_thresholds["eye_bad"] or slouch_angle > calibrated_thresholds["slouch_bad"]:
                    posture_status = "Hunched Forward"
                    color = (0, 0, 255)
                elif z_diff_head_nose > calibrated_thresholds["nose_warn"] or z_diff_head_eye > calibrated_thresholds["eye_warn"] or slouch_angle > calibrated_thresholds["slouch_warn"]:
